...
class MPCcontroller():
	...
	def __init__(self, ts = 0.1, P = np.ones((12,1)),   \
		...
		self.r = np.zeros((3,1))#xbar_v[0:3].reshape((3,1))#

		P = np.eye(12)#np.diag(P)
		Q = np.eye(12)#np.diag(Q)
		R = np.eye(4)#np.diag(R)
		...
		xbar.value = np.zeros((12,1)).flatten()#xbar_v.flatten()
		ubar = cvx.Parameter(4)
		ubar.value = np.zeros((4,1)).flatten()#ubar_v.flatten()
